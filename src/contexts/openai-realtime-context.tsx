...
const OpenAIRealtimeProvider: React.FC<OpenAIRealtimeProviderProps> = ({
  ...
  const addListener = useCallback(
    (event: RealtimeEvent, callback: (data: unknown) => void) => {
      ...
      console.log(
        '[OpenAI] Total listeners for',
        event,
        ':',
        eventListeners.current.get(event)?.size
      );
    },
    []
  );
    ...
    (data: ArrayBuffer | Blob) => {
      if (connection && connection.readyState === WebSocket.OPEN) {
        if (data instanceof Blob) {
          console.log(
            '[OpenAI] Sending audio data (Blob):',
            data.size,
            'bytes'
          );
          data.arrayBuffer().then((buffer) => {
            ...
            if (audioChunksSentRef.current % 50 === 0) {
              console.log(
                '[OpenAI] Sent',
                audioChunksSentRef.current,
                'audio chunks so far. Last chunk:',
                base64Audio.length,
                'chars'
              );
            }
          ...
          console.log(
            '[OpenAI] Sending audio data (ArrayBuffer):',
            data.byteLength,
            'bytes'
          );
          const base64Audio = btoa(
        ...
        console.warn(
          '[OpenAI] Cannot send audio: connection not open. State:',
        ...
        ws.onmessage = (event) => {
          try {
            ...
            if (message.type === RealtimeEvent.Transcript) {
              ...
              message.type === RealtimeEvent.InputAudioBufferCommitted
            ) {
              console.log(
                '[OpenAI] ✅ Audio buffer committed - item_id:',
                message.item_id
              );
              emit(RealtimeEvent.InputAudioBufferCommitted, message);
            } else if (
              message.type === RealtimeEvent.InputAudioBufferSpeechStarted
            ) {
              console.log(
                '[OpenAI] 🎙️ Speech detected starting - item_id:',
                message.item_id
              );
              emit(RealtimeEvent.InputAudioBufferSpeechStarted, message);
            } else if (
              message.type === RealtimeEvent.InputAudioBufferSpeechStopped
            ) {
              console.log(
                '[OpenAI] 🔇 Speech detected stopping - item_id:',
                message.item_id
              );
  ...
  const commitAudioBuffer = useCallback(() => {
    if (connection && connection.readyState === WebSocket.OPEN) {
      ...
      console.log(
        '[OpenAI] Total audio chunks sent:',
        audioChunksSentRef.current
      );
      const commitMessage = {
...
export type { SessionConfig, TranscriptionData };
